...
dagre.layout = (nodes, edges, layout, state) => {
    ...
    const order = (g) => {
        ...
        const initOrderDfs = (g) => {
            const nodes = Array.from(g.nodes.values()).filter((node) => !g.hasChildren(node.v));
            let maxRank = -1;
            for (const node of nodes) {
                const r = node.label.rank;
                if (r !== undefined && r > maxRank) {
                    maxRank = r;
                }
            }
            if (maxRank < 0) {
            ...
            const layers = Array.from(new Array(maxRank + 1), () => []);
            const visited = new Set();
            const seenInLayer = layers.map(() => new Set());
            const pushLayer = (v) => {
                const node = g.node(v);
                if (!node || node.label.rank === undefined) {
                    return;
                }
                const rank = node.label.rank;
                const seen = seenInLayer[rank];
                if (!seen.has(v)) {
                    seen.add(v);
                    layers[rank].push(v);
                }
            ...
            const dfs = (v) => {
                ...
                visited.add(v);
                pushLayer(v);
                const successors = Array.from(g.successors(v).keys());
            ...
            const sources = nodes
                ...
                .sort((a, b) => a.label.rank - b.label.rank)
                .map((node) => node.v);
            for (const source of sources) {
                dfs(source);
            }
            for (const node of nodes) {
                if (!visited.has(node.v)) {
                    dfs(node.v);
                }
            }
            return layers;
